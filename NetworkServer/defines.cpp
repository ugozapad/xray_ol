...
const char *NAME_OF_THE_APP = "XRay multicast server";
...
//        if (HW.pDDraw && Device.bFS) HW.pDDraw->FlipToGDISurface();
...
ENGINE_API void __fastcall _verify     (const char *expr, char *file, int line) {
        ...
        sprintf(_tmp_buf,"Assertion failed in file %s, line %d.\nExpression was '%s'",
                file, line, expr);
                Log(_tmp_buf);
//        if (HW.pDDraw && Device.bFS) CHK_DX(HW.pDDraw->FlipToGDISurface());
